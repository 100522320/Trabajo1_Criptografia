...
import socket # Para la comunicación
...
'''def start_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True: # Bucle para aceptar conexiones
            conn, addr = s.accept()
            with conn:
                logger.info(f"[Servidor] Conectado con {addr}")
                while True: # Bucle para recibir datos de esta conexión
                    data = conn.recv(1024) # Recibir hasta 1024 bytes
                    if not data:
                        logger.info(f"[Servidor] Cliente {addr} desconectado.")
                        break # El cliente cerró la conexión
                    
                    logger.info(f"[Servidor] Petición recibida: {data.decode('utf-8')[:50]}...")
