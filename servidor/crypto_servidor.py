...
# Variables globales para las claves RSA del servidor
...
def guardar_cita(usuario: str, fecha: datetime, motivo_encriptado: str) -> bool:
    """Guarda una cita cifrada en el archivo JSON"""
    try:
        citas = load_citas()
...
def obtener_cita(usuario: str, fecha: datetime) -> str:
    """Obtiene una cita cifrada del archivo JSON"""
    citas = load_citas()
...
def encriptar_mensaje(clave: bytes, mensaje: str) -> str:
    """Cifra un mensaje para comunicación con el cliente (AES-GCM)"""
    try:
...
def desencriptar_mensaje(clave: bytes, mensaje_cifrado: str) -> str:
    """Descifra un mensaje recibido del cliente (AES-GCM)"""
    try:
